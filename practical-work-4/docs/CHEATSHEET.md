...
# Автоматическая проверка ВСЕХ требований (запустит Colima автоматически)
...
# Без очистки контейнеров (оставить запущенными)
...
💡 **Совет:** Запустите `./scripts/full-validation.sh` перед защитой, чтобы убедиться что всё работает!
